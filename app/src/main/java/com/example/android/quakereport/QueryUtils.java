...
public final class QueryUtils {
    ...
    public static List<Earthquake> extractEarthquakes(String earthquakeJson) {
        ...
        try {
            ...
            JSONArray arrayF = object.getJSONArray("features");
            for (int i=0;i<arrayF.length();i++)
            {
                JSONObject Feat = arrayF.getJSONObject(i);
                JSONObject pro = Feat.getJSONObject("properties");
                double magn = pro.getDouble("mag");
                String plac = pro.getString("place");
                long tim = pro.getLong("time");
                String url = pro.getString("url");
                Earthquake earthquake = new Earthquake(magn,plac,tim,url);
